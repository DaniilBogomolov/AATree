...
public class AATree<T extends Comparable<T>> {
    ...
    private Node<T> skew(Node<T> root) {
       // Check if root isn't null
        if (root == null) {
            ...
            // If we don't have a left sibling -> we don't have any left child -> return our root
        } else if (root.left == null) {
            return root;
            // Otherwise, if the left child and the root are on the same level, we need to reverse a subtree
        } else if (root.left.level == root.level) {
            // Take a link to the left sibling root
            Node leftNode = root.left;
            // The left sibling's root right child becomes the left child of initial root
            root.left = leftNode.right;
            // Link the left sibling's root to initial root as a right child
            leftNode.right = root;
            // Now the leftNode becomes new root -> we return leftNode and assign it to root in insert method
            return leftNode;
            // If the left child isn't on the same level as root then we just return this root
        } else {
            return root;
        }
    }
    ...
    private Node<T> split(Node<T> root) {
        ...
        if (root == null) {
            return null;
            // If it isn't then we check whether we have a right child and right grandchild
            // If some of them isn't present then we don't have to do anything and rebalance the tree
        } else if (root.right == null || root.right.right == null) {
            return root;
            // Otherwise, we check whether the root level equals to right grandchild level
            // If it is
    ...
    private Node<T> remove(T value, Node<T> root) {
        ...
        if (root == null) {
            ...
            if (root.left == null && root.right == null) {
                return null;
                // If it isn't a leaf then we try to find it's successor or predecessor to swap
